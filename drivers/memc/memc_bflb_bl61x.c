...
#define EFUSE_PSRAM_SIZE_SIZE 3
#define EFUSE_FLASH_SIZE_POS 26
#define EFUSE_FLASH_SIZE_SIZE 7
...
static int memc_bflb_bl61x_get_psram_ctrl(void)
{
	uint32_t tmp;
	volatile int wait_ack = PSRAM_CONFIG_WAIT;

	tmp = sys_read32(PSRAM_CONFIG_ADDR + PSRAM_CONFIGURE_OFFSET);
	tmp &= PSRAM_REG_CONFIG_REQ_UMSK;
	tmp |= 1 << PSRAM_REG_CONFIG_REQ_POS;
	sys_write32(tmp, PSRAM_CONFIG_ADDR + PSRAM_CONFIGURE_OFFSET);
...
static void memc_bflb_bl61x_release_psram_ctrl(void)
{
	uint32_t tmp;
...
static int memc_bflb_bl61x_get_psram_reg(uint8_t reg, uint16_t *out)
{
	uint32_t tmp;
	volatile int wait_ack = PSRAM_CONFIG_WAIT;
	int err;

	err = memc_bflb_bl61x_get_psram_ctrl();
	if (err < 0) {
	...
	tmp = sys_read32(PSRAM_CONFIG_ADDR + PSRAM_MANUAL_CONTROL_OFFSET);
	*out = (uint16_t)(tmp >> 16);

	memc_bflb_bl61x_release_psram_ctrl();
...
static int memc_bflb_bl61x_init_psram(const struct device *dev)
{
	...
	tmp = sys_read32(PSRAM_CONFIG_ADDR + PSRAM_ROUGH_DELAY_CTRL5_OFFSET);
	tmp &= PSRAM_REG_ROUGH_SEL_I_DQS0_UMSK;
	tmp |= dqs_d_t << PSRAM_REG_ROUGH_SEL_I_DQS0_POS;
	sys_write32(tmp, PSRAM_CONFIG_ADDR + PSRAM_ROUGH_DELAY_CTRL5_OFFSET);
	...
	tmp = sys_read32(PSRAM_CONFIG_ADDR + PSRAM_MANUAL_CONTROL2_OFFSET);
	...
	sys_write32(tmp, PSRAM_CONFIG_ADDR + PSRAM_MANUAL_CONTROL2_OFFSET);
	...
	sys_write32(tmp, PSRAM_CONFIG_ADDR + PSRAM_WINBOND_PSRAM_CONFIGURE_OFFSET);
	...
	err = memc_bflb_bl61x_get_psram_reg(0, &check_dat);
	if (err < 0) {
...
static int memc_bflb_bl61x_init(const struct device *dev)
{
	...
	psram_size = (dev_infos >> EFUSE_PSRAM_SIZE_POS) & EFUSE_PSRAM_SIZE_SIZE;
	if (psram_size) {
		switch (psram_size) {
			case 1:
			data->psram_size = 0x400000;
			break;
			case 2:
			data->psram_size = 0x800000;
			break;
			case 3:
			data->psram_size = 0x1000000;
			break;
			default:
			data->psram_size = 0;
			break;
	...
	flash_size = (dev_infos >> EFUSE_FLASH_SIZE_POS) & EFUSE_FLASH_SIZE_SIZE;
	if (flash_size) {
		switch (flash_size) {
			case 1:
			data->flash_size = 0x200000;
			break;
			case 2:
			data->flash_size = 0x400000;
			break;
			case 3:
			data->flash_size = 0x600000;
			break;
			case 4:
			data->flash_size = 0x800000;
			break;
			default:
			data->flash_size = 0;
			break;
	...
	if (clock_control_get_status(clock, (clock_control_subsys_t)BL61X_CLKID_CLK_WIFIPLL)
		== CLOCK_CONTROL_STATUS_ON) {
		memc_bflb_bl61x_init_psram_clock(dev, 0);
	} else if (clock_control_get_status(clock, (clock_control_subsys_t)BL61X_CLKID_CLK_AUPLL)
		== CLOCK_CONTROL_STATUS_ON) {
	...
	if (data->psram_size != 0x400000) {
		LOG_ERR("Only existing 4MB Winbond X8 PSRAM Config is supported:\
			Other configurations have not been published");
		return -ENOTSUP;
	}
