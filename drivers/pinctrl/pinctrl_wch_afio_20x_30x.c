...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	for (i = 0; i < pin_cnt; i++, pins++) {
		uint8_t port = (pins->config >> CH32V20X_V30X_PINCTRL_PORT_SHIFT) & 0x07;
		uint8_t pin = (pins->config >> CH32V20X_V30X_PINCTRL_PIN_SHIFT) & 0x0F;
		uint8_t bit0 = (pins->config >> CH32V20X_V30X_PINCTRL_RM_BASE_SHIFT) & 0x1F;
		uint8_t pcfr_id = (pins->config >> CH32V20X_V30X_PINCTRL_PCFR_ID_SHIFT) & 0x01;
		uint8_t remap = (pins->config >> CH32V20X_V30X_PINCTRL_RM_SHIFT) & 0x3;
