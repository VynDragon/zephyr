...
static void clock_control_bl61x_set_wifipll_source(uint32_t source)
{
	...
	if (source == 0) {
		tmp = (tmp & GLB_WIFIPLL_REFCLK_SEL_UMSK) | (3 << GLB_WIFIPLL_REFCLK_SEL_POS);
	} else if (source == 1) {
		tmp = (tmp & GLB_WIFIPLL_REFCLK_SEL_UMSK) | (1 << GLB_WIFIPLL_REFCLK_SEL_POS);
	}
...
static uint32_t clock_control_bl61x_get_bclk(const struct device *dev)
{
	...
	clock = clock_control_bl61x_get_hclk(dev);
	return clock / (tmp + 1);
}
...
static void clock_control_bl61x_init_root_as_wifipll(const struct device *dev)
{
	...
	if (data->wifipll.overclock) {
		clock_control_bl61x_init_wifipll(bl61x_pll_configs_O480M, data->wifipll.source, config->crystal_frequency);
	} else {
	...
	if (data->wifipll.source == bl61x_clkid_clk_crystal) {
		clock_control_bl61x_set_root_clock(3);
	} else {
		clock_control_bl61x_set_root_clock(2);
	}
}
...
static int clock_control_bl61x_update_root(const struct device *dev)
{
	...
	clock_control_bl61x_set_root_clock(0);
	if (clock_control_bl61x_set_root_clock_dividers(0, 0) != 0) {
		return -EIO;
	}
	...
	clock_control_bl61x_set_machine_timer_clock(1, 0, clock_control_bl61x_mtimer_get_xclk_src_div(dev));
...
static void clock_control_bl61x_uart_set_clock(bool enable, uint32_t clock, uint32_t divider)
{
	...
	if (clock < 2) {
		...
		tmp = (tmp & HBN_UART_CLK_SEL2_UMSK) | (0 << HBN_UART_CLK_SEL2_POS);
	} else {
		tmp = (tmp & HBN_UART_CLK_SEL_UMSK) | (0 << HBN_UART_CLK_SEL_POS);
		tmp = (tmp & HBN_UART_CLK_SEL2_UMSK) | (1 << HBN_UART_CLK_SEL2_POS);
	}
	sys_write32(tmp, HBN_BASE + HBN_GLB_OFFSET);
...
static struct clock_control_bl61x_data clock_control_bl61x_data = {
	...
	.wifipll_enabled = DT_NODE_HAS_STATUS_OKAY(DT_INST_CLOCKS_CTLR_BY_NAME(0, wifipll)),
	.aupll_enabled = DT_NODE_HAS_STATUS_OKAY(DT_INST_CLOCKS_CTLR_BY_NAME(0, aupll)),

	.root = {
#if CLK_SRC_IS(root, wifipll)
		.source = bl61x_clkid_clk_wifipll,
#elif CLK_SRC_IS(root, aupll)
...
BUILD_ASSERT(CLK_SRC_IS(aupll, crystal) || CLK_SRC_IS(wifipll, crystal) || CLK_SRC_IS(root, crystal) ? DT_NODE_HAS_STATUS_OKAY(DT_INST_CLOCKS_CTLR_BY_NAME(0, crystal)) : 1,
	     "Crystal must be enabled to use it");

BUILD_ASSERT(CLK_SRC_IS(root, wifipll) ? DT_NODE_HAS_STATUS_OKAY(DT_INST_CLOCKS_CTLR_BY_NAME(0, wifipll)) : 1,
	     "Wifi PLL must be enabled to use it");

BUILD_ASSERT(CLK_SRC_IS(root, aupll) ? DT_NODE_HAS_STATUS_OKAY(DT_INST_CLOCKS_CTLR_BY_NAME(0, aupll)) : 1,
