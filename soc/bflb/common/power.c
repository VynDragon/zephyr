...
#define BL616CL_BO_THRES_MIN	2330000
...
static void power_bflb_setup_bor(const struct device *dev)
{
	...
	tmp |= ((config->bo_threshold - E907_VALID_BO_ID_MAX) << HBN_BOD_TH_AON_POS) & HBN_BOD_TH_AON_MSK;
	/* enable BOD */
...
BUILD_ASSERT(DT_INST_ENUM_IDX(0, brown_out_threshold_microvolt) < E907_VALID_BO_ID_MAX,
	     "BL61x/BL80x Brown Out threshold must be  2050000 uV > value > 2400000 uV \
excluding 2330000 and 2370000 uV");
#elif defined(CONFIG_SOC_SERIES_BL616CL)
BUILD_ASSERT(DT_INST_ENUM_IDX(0, brown_out_threshold_microvolt) >= E907_VALID_BO_ID_MAX,
